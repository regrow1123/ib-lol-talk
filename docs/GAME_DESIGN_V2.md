...
정확한 수치 시뮬레이션이 아니라, 플레이어와 상대방의 **의도 충돌**에서 재미가 나온다.
...
- 피해량은 LLM이 스킬 특성 + 현재 상태를 참고하여 판정 (섹션 11 참조)
...
### 위치: 상황 태그
그리드 좌표(x,y) 제거. 대신 **상황 기반 위치** (거리 단위: 1티모 = 100유닛):
...
- "미니언 뒤에서 안전하게 CS 먹기" → 파밍 결과 + 상대방 반응 → 1턴
- "부쉬 숨어서 기습 노리기" → 대기 + 상대방이 어떻게 대응하는지 → 1턴
- "E1 범위 들어가서 압박" → 전진 + 교전 발생 여부 → 1턴
...
### 상대방도 의도 기반 행동
상대방은 플레이어 의도를 **읽고 대응**:
- 플레이어 Q1 → 상대방이 미니언 뒤로 이동 (Q1 차단) → "실전에서도 Q1은 미니언 뒤에서 피함"
- 플레이어 CS → 상대방이 그 타이밍에 트레이드 → "막타 순간이 가장 취약한 타이밍임"
- 플레이어 부쉬 → 상대방이 웨이브 푸시 → "부쉬 견제는 웨이브 컨트롤로 대응함"
- 플레이어 올인 → 상대방이 W1 쉴드 + 반격 콤보 → "올인 들어오면 W1로 버티고 카운터함"
...
- **패시브 (연타)**: 스킬 사용 후 AA 2회 공속 증가 + 기력 회복

### 스킬 레벨업: 오버레이 UI
레벨업 시 오버레이로 스킬 선택:
- Q/W/E/R 버튼 + 스킬 설명 표시
- 선택 완료 전까지 게임 일시 정지
- R은 레벨 6/11/16에서만 선택 가능
- 클라이언트에서 처리 (서버 호출 불필요)

---

## 5. 소환사 주문 & 룬

### 소환사 주문 (게임 시작 시 2개 선택)
5개 중 2개를 자유롭게 선택:
- **점멸 (Flash)**: 즉시 이동, 위기 탈출/기습. 긴 쿨타임.
...
### UI 구조 (유지)
- KakaoTalk 스타일 채팅 (노란 말풍선 = 나, 흰 말풍선 = 상대방)
- 서술은 시스템 메시지(카톡 날짜 구분선 형태)로 표시
- 상단 상태바 (HP%, 기력, CS, 레벨, 쿨타임)
- 하단 텍스트 입력 + 추천 선택지 칩
- 셋업 화면 (스펠/룬 선택)
...
- **패시브 (연타)**: 스킬 사용 후 AA 2회 공속 40%↑ + 기력 회복. 콤보 사이에 AA 끼워넣으면 효율적
