...
### 핵심 경험
1. 자연어로 의도를 입력하면 AI가 읽고 대응 → **진짜 읽기 싸움**
2. 교전 과정에서 스킬 메커니즘이 자연스럽게 서술 → **플레이하면서 배움**
3. AI가 왜 그렇게 대응했는지 설명 → **실전 지식 축적**
...
```json
{
  ...
  "passive": {
    "name": "연타",
    "nameEn": "Flurry",
    "description": "스킬 사용 후 다음 2회 기본공격 공격속도 40% 증가 + 에너지 회복"
  },
  "skills": {
    "Q": {
      "name": ["음파", "공명타"],
  ...
  "tips": {
    "combos": [
      "Q1 → Q2 → AA → E1 → AA → R (기본 올인 콤보)",
      "W1(미니언) → R → Q1 → Q2 (인섹 킥)",
      "E1 → E2(둔화) → Q1 → Q2 (둔화 후 Q 적중률 상승)"
    ],
    "strengths": ["초반 교전 강함", "높은 기동성", "다양한 콤보"],
    "weaknesses": ["후반 약화", "에너지 관리 중요", "Q 빗나가면 올인 불가"]
  }
}
```
...
### 4.2 라인전
1. 플레이어가 자연어로 의도 입력
2. LLM이 의도 해석 + AI 대응 결정 + 결과 판정
3. 서술 + AI 코멘트 + 상태 업데이트 + 추천 선택지
4. 레벨업 시 스킬 레벨업 오버레이
5. 반복
...
### 끼어들기 (Interrupt)
- 플레이어 저강도 + AI 고강도 → 턴 중단, 다음 턴에 대응 기회 부여
...
### 7.2 행동 원칙
- **플레이어 의도를 읽고 대응** (랜덤 아님)
- 대응 이유를 설명 → 교육적 가치
- **스킬의 모든 효과를 최대한 활용**하여 행동 → 플레이어가 스킬 효과를 확실히 체험
  - 예: Q1 맞추면 반드시 Q2 돌진도 보여줌 (Q1만 쓰고 끝내지 않음)
  - 예: W를 미니언에 사용해서 위치 변경 수단으로 활용
  - 예: E1 적중 후 E2 둔화로 추격/도주에 활용
  - 예: R로 타워 쪽으로 차서 킬각 만들기
  - 예: 패시브(연타) 활용 — 스킬 사이에 AA 끼워넣어 에너지 회복
- 다양한 스킬 조합 적극 활용 (같은 패턴 반복 X)
- 적당히 공격적 (파밍만 하지 않음)
- 이기는 것보다 **플레이어가 배우는 것**이 목표

### 7.3 챔피언별 AI 행동
AI도 선택된 챔피언의 스킬/특성에 맞게 행동:
