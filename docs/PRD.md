...
- LLM이 매 턴 `elapsed`를 반환: 이번 턴의 시간 규모
  - `"short"` = 2초 (교전, 스킬 교환)
  - `"medium"` = 5초 (CS 몇 개 + 소규모 행동)
  - `"long"` = 10초 (파밍 구간, 대기)
- 서버가 `elapsed`를 초로 변환하여 **쿨다운 차감, 자원 회복** 등 시간 경과 계산
- 데미지 엔진은 **즉시 효과** 계산 (스킬 데미지, 쉴드량, 자원 소모)

**왜 이 방식인가**: 플레이어의 한 선택이 "Q1으로 견제"일 수도, "미니언 정리하면서 CS 3개 먹기"일 수도 있다. LLM이 정확한 초를 추정하는 건 어렵지만, "짧다/보통/길다" 3단계 판단은 확실히 할 수 있다. 서버가 고정 매핑으로 환산하므로 수치 일관성도 보장된다.
...
- 에너지: 초당 50 자연회복 (예: elapsed="short"(2초) → +100 회복)
