...
### 시간 모델: 비고정 간격 스냅샷
- **턴 간 시간 간격은 고정이 아님** — 전투 중이면 짧고(1~2초), 파밍 중이면 길 수 있음(5~10초)
- 하나의 턴에 **여러 액션이 포함**될 수 있음 (콤보, 양쪽 교환 등)
- **서버는 스냅샷 저장소** — "이 턴이 끝난 후의 상태"만 기록
- **LLM이 다음 상태를 결정** — 쿨다운, 거리, 자원 등 시간 경과가 반영된 값을 직접 반환
- 서버는 LLM이 반환한 쿨다운/자원 값을 **그대로 저장** (자체 차감/계산 없음)
- 데미지 엔진은 **즉시 효과만 계산** (스킬 데미지, 쉴드량 등 — 시간 경과 불필요한 것)
...
#### blocked (boolean)
두 챔프를 잇는 직선 경로에 미니언이 있는지 여부.
- `true` → 직선 경로에 미니언 존재 → Q1 같은 투사체가 미니언에 막힘
- `false` → 직선 경로에 미니언 없음 → 투사체 직통 (다른 위치에 미니언이 있어도 무관)
- E1(자기 주변 범위), R/AA(대상지정)는 blocked 무관
- LLM이 양측 위치/행동에 따라 매 턴 판정
...
### 5.2 클라이언트 필터링
- 습득 여부 + 쿨다운 기준 필터링
- 사거리 밖 스킬 → "(사거리 밖)" 표시 + 흐리게 (선택은 가능하되 정보 제공)
- 필터 후 **최대 3개** 표시
- 이모지 사용 금지
...
- `cs`: 이번 턴에 획득한 CS 수
- `playerCooldowns` / `enemyCooldowns`: 턴 종료 시점 쿨다운 상태 (초 단위)
- `playerResource` / `enemyResource`: 턴 종료 시점 자원 (사용+회복 반영)
- `enemySkillUp`: 적 레벨업 시 스킬 선택 (상황 기반)
...
### 서버가 하지 않는 것 (LLM이 관리)
- ~~쿨다운 적용/차감~~ → LLM이 쿨다운 값 직접 반환
- ~~자원 소모/회복~~ → LLM이 자원 값 직접 반환
- 턴 간 시간 경과 계산 (고정 간격 없으므로)

### 가드레일 (서버 검증)
- HP: 0~maxHp 클램프
- 자원: 0~maxResource 클램프 (LLM 반환값에 대해)
- CS/레벨: 감소 불가
- 쿨다운: 0 이상 클램프 (LLM 반환값에 대해)
- distance: 0 이상
- 미습득 스킬 사용 시 해당 action 무시
