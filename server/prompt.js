...
function buildStateBlock(state, champ) {
...
// ===== Shared rules (used in both plan and resolve) =====

function buildSharedRules(champ) {
  return `LoL 1v1 라인전 텍스트 전략 게임. 양쪽 ${champ.name}(${champ.nameEn}) 미러매치.
...
export function buildPlanPrompt(gameState) {
  ...
  const staticPart = `${buildSharedRules(champ)}

## 역할: 행동 계획 생성
너는 양측의 다음 행동을 생성하는 전략 플래너야.
- 플레이어 행동 3개 (우선순위순, 최선 먼저)
- 적 행동 1개 (현재 상태 기반 최선 행동)

## AI(적) 행동 원칙
- 동등한 상대. 절대 봐주지 않음. 회피/반격/맞교환 적극 응수
- HP/쿨다운/거리/CS차/자원 기반 동적 판단
- 다양한 스킬 조합·전략 적극 사용. 같은 패턴 반복 금지
- 미습득/쿨다운중/자원부족 스킬 사용 금지
- 적에게 skillPoints > 0이면 반드시 enemySkillUp 선택

## 행동 텍스트 규칙
- 1인칭 구어체로 작성 (본인이 직접 말하는 행동 선언문)
  플레이어 예: "상대 Q 쿨 돌았으니 Q1 꽂아볼까" (O) / "Q1으로 견제하세요" (X)
  적 예: "음파 한 발 꽂아줄까" / "CS 좀 먹어야지"
- 근거를 자연스럽게 포함: "쉴드 없을 때 한 대 때려야지", "미니언 정리하면서 렙업 노려보자"
- 이모지 금지
- requires: 실행에 필요한 핵심 스킬 키 (없으면 null)

## JSON 응답 (반드시 valid JSON만, 마크다운 감싸기 금지)
{
  "playerActions": [
    {"action":"행동 설명","skills":["Q1"],"target":"enemy","requires":"Q","text":"한국어 1인칭 행동 선언"},
    {"action":"행동 설명","skills":[],"target":null,"requires":null,"text":"한국어 1인칭 행동 선언"},
    {"action":"행동 설명","skills":["W1"],"target":"self","requires":"W","text":"한국어 1인칭 행동 선언"}
  ],
  "enemyAction": {"action":"행동 설명","skills":["Q1"],"target":"player","text":"한국어 1인칭 행동 선언"},
  "enemySkillUp": null|"Q"|"W"|"E"
}`;

  const dynamicPart = buildStateBlock(gameState, champ);
...
## 역할: 결과 판정
너는 심판이야. 양쪽 행동이 확정됐고, 무슨 일이 일어났는지 판정해.
...
## CS 규칙 보충
- 레벨업 발생 시에도 정상 응답 (서버가 레벨업 처리)

## JSON 응답 (반드시 valid JSON만, 마크다운 감싸기 금지)
{
  "narrative": "한국어 전투 나레이션 1~2문장. 스킬 효과를 교육적으로 자연스럽게 설명",
  "aiChat": "적 챔피언이 올챗으로 하는 말",
  ...
  // Build the action context for the resolve call
  let actionContext;
  if (isFreeText) {
    actionContext = `## 이번 턴 행동
플레이어 자유 입력: "${playerAction}"
적 확정 행동: ${JSON.stringify(enemyAction)}

플레이어 자유 입력을 해석하여 의도된 행동을 판단하고, 적 행동과 함께 결과를 판정해.
자유 입력이 스킬 사용이 아닌 경우(대화, 질문 등)에도 적 행동은 그대로 실행된다.`;
  } else {
    actionContext = `## 이번 턴 행동
플레이어 선택 행동: ${JSON.stringify(playerAction)}
적 확정 행동: ${JSON.stringify(enemyAction)}

양쪽 행동이 동시에 실행된다. 결과를 판정해.`;
  }
