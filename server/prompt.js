...
\`\`\`json
{
  ...
  "suggestions": ["다음 턴에 할 수 있는 자연어 선택지 3개. 현재 상황에 맞는 구체적 행동. 짧게 (예: 'Q로 견제', 'CS 먹으면서 대기', 'E 범위 안으로 접근')"]
}
\`\`\`
...
- suggestions는 반드시 3개. 현재 상황에서 실제로 가능한 행동만. 사거리/쿨다운/에너지 고려. 짧고 자연스러운 한국어.
`;
}
